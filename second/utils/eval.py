...
def get_my_eval_result(gt_annos,
                       dt_annos,
                       current_classes,
                       difficultys=[0, 1, 2],
                       z_axis=1,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            detail[class_name][f"bev@{min_overlaps[i, 1, j]:.2f}"] = mAPbev[0]
            detail[class_name][f"3d@{min_overlaps[i, 2, j]:.2f}"] = mAP3d[0]

            iou_th = min_overlaps[i, :, j]
            result += print_str("{} AP(Average Precision)@{:.2f}, {:.2f}".format(class_to_name[curcls],
                                                                                 iou_th[1],
                                                                                 iou_th[2]))
            aPbev_str = "{:.2f}".format(mAPbev[0])
            aP3d_str = "{:.2f}".format(mAP3d[0])
            result += print_str(f"bev  AP:{aPbev_str}")
            result += print_str(f"3d   AP:{aP3d_str}")

            if (iou_th[1] >= 0.45) and (iou_th[1] <= 0.55):
                mAp += mAPbev[0]
    mAp = mAp / len(current_classes)
    return {
        "result": result,
        "detail": detail,
        "mAp": mAp
