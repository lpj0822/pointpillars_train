...
class PointPillarsLoss(nn.Module):
    ...
    def __init__(self, target_assigner, nms_score_thresholds, nms_iou_thresholds,
                 nms_pre_max_sizes, nms_post_max_sizes,
                 loc_loss_ftor, cls_loss_ftor,
                 loc_loss_weight, cls_loss_weight):
        super().__init__()
    ...
    def loss(self, example, preds_list):
        box_preds = preds_list[0]
        cls_preds = preds_list[1]
    ...
    def forward(self, example, preds_list):
        batch_anchors = example["anchors"]
        batch_size = batch_anchors.shape[0]
        # need to check size.
        box_preds = preds_list[0].view(batch_size, -1, self._box_coder.code_size)
        err_msg = f"num_anchors={batch_anchors.shape[1]}, " \
            f"but num_output={box_preds.shape[1]}. please check size"
        assert batch_anchors.shape[1] == box_preds.shape[1], err_msg

        if self.training:
            return self.loss(example, preds_list)
        else:
            self.start_timer("predict")
            with torch.no_grad():
                res = self.predict(example, preds_list)
            self.end_timer("predict")
            return res

    def predict(self, example, preds_list):
        ...
        t = time.time()
        batch_box_preds = preds_list[0]
        batch_cls_preds = preds_list[1]
