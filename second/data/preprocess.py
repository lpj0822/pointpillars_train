...
def prep_remove_environment(remove_environment, class_names,
                            gt_dict, points):
...
def prep_remove_unknown(remove_unknown, gt_dict):
...
def prep_pointcloud(input_dict,
    ...
    if training:
        ...
        if use_group_id and "group_ids" in anno_dict:
            print("use_group_id", use_group_id)
    ...
    if "calib" in input_dict:
        calib = input_dict["calib"]
    prep_remove_outside_points(input_dict, reference_detections,
                               remove_outside_points, points)
    if training:
        prep_remove_environment(remove_environment, class_names, gt_dict, points)
    ...
    if training:
        ...
        bev_map = simplevis.my_dataset_vis(points, boxes_lidar)
        cv2.imshow('pre-noise', bev_map)
        cv2.waitKey(0)
        '''


        prep_remove_unknown(remove_unknown, gt_dict)
